...
namespace FireTechHMS.Forms
{
    public partial class ConfigPanel : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            int p = Config.updateConfigurations(hostel_name, keymoney, auth_key, ac_rent, non_ac_rent, ac_count, non_ac_count, smtp_mail, smtp_port, smtp_password, hostel_mail, max_std_count, max_bed_count);
            if (p == 1)
            {
                new MessageContainer("Configured Successfully !").Show();
            }
            else if (p == 0)
            {
                new MessageContainer("Configuration failed !").Show();
            }
        }
    }
}
